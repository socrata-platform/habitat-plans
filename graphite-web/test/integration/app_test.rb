...
describe command("PYTHONPATH=#{pkg_path}/webapp #{pip_path} show " \
                 'graphite-web') do
  its(:exit_status) { should eq(0) }
  its(:stdout) { should match(/^Version: 0\.9\.12$/) }
  its(:stdout) { should match(%r{^Location: #{pkg_path}/webapp$}) }
end
...
%w[content graphite].each do |d|
  describe directory(File.join(pkg_path, 'webapp', d)) do
    it { should exist }
